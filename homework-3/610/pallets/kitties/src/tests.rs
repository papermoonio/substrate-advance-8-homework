...
fn it_works_kitty_id_overflow() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(Kitties::create(caller), Error::<Test>::KittyIdOverflowError);
    });
}
...
fn it_works_kitty_breed_with_same_kitty_id() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Kitties::breed(caller.clone(), KITTY_ID, KITTY_ID),
            Error::<Test>::SameKittyIdError
        );
    });
}
...
fn it_works_kitty_breed_ok() {
    new_test_ext().execute_with(|| {
        ...
        let current_kitty_id = crate::NextKittyId::<Test>::get();
        assert_eq!(current_kitty_id, 2);
        assert_ok!(Kitties::breed(caller.clone(), KITTY_ID, KITTY_ID2));

        let kitty = crate::Kitties::<Test>::get(current_kitty_id).unwrap();
...
fn it_works_kitty_sale_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Kitties::sale(bob_caller.clone(), KITTY_ID, 10),
            Error::<Test>::NotOwnerError
        );
    });
}
...
fn it_works_kitty_bid_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            crate::KittiesBid::<Test>::get(KITTY_ID).unwrap()[0],
            (bid_account_2, 200u128)
        );
    });
}
...
fn it_works_kitty_bid_not_sale() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Kitties::bid(bid_caller.clone(), KITTY_ID, 100),
            Error::<Test>::KittyNotOnSaleError
        );
    });
}
...
fn it_works_kitty_transfer() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            crate::KittyOwner::<Test>::get(KITTY_ID),
            Some(buyer_account)
        );
